...
const QuizGameRoom = ({ roomCode, playerId, playerName, onLeave }) => {
  ...
  useEffect(() => {
    ...
    if (roomCode && playerId && playerName) {
      console.log('🚪 Joining quiz room:', { roomCode, playerId, playerName });
    ...
    socket.on('quiz-started', (data) => {
      ...
      const systemMessage = {
        id: Date.now().toString(),
        type: 'system',
        text: `Quiz started! Question ${data.questionNumber}/5`,
        timestamp: new Date()
      };
      setMessages(prev => [...prev, systemMessage]);
    });
    ...
    socket.on('question-answered', (data) => {
      ...
      const systemMessage = {
        id: Date.now().toString(),
        type: 'system',
        text: `${data.playerName} answered: ${data.answerText} - ${isCorrect ? '✅ Correct!' : '❌ Wrong'}`,
        timestamp: new Date()
      };
      setMessages(prev => [...prev, systemMessage]);
    });
